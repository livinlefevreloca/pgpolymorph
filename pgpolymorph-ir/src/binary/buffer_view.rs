...
impl<'a> BufferView<'a> {
    ...
    fn peek_bytes(&self, n: usize) -> Result<&'a [u8]> {
        if self.remaining() < n {
            return Err(Error::UnexpectedEof {
                expected: n,
                available: self.remaining(),
    ...
    pub(crate) fn read_u8_exact(&mut self, len: usize) -> Result<u8> {
        self.ensure_exact_remaining(len)?;
        self.read_u8()
    }

    pub(crate) fn read_i16_exact(&mut self, len: usize) -> Result<i16> {
        self.ensure_exact_remaining(len)?;
        self.read_i16()
    }

    pub(crate) fn read_i32_exact(&mut self, len: usize) -> Result<i32> {
        self.ensure_exact_remaining(len)?;
        self.read_i32()
    }

    pub(crate) fn read_i64_exact(&mut self, len: usize) -> Result<i64> {
        self.ensure_exact_remaining(len)?;
        self.read_i64()
    }

    pub(crate) fn read_u32_exact(&mut self, len: usize) -> Result<u32> {
        self.ensure_exact_remaining(len)?;
        self.read_u32()
    }

    pub(crate) fn read_f32_exact(&mut self, len: usize) -> Result<f32> {
        self.ensure_exact_remaining(len)?;
        self.read_f32()
    }

    pub(crate) fn read_f64_exact(&mut self, len: usize) -> Result<f64> {
        self.ensure_exact_remaining(len)?;
        self.read_f64()
    }
    ...
    pub(crate) fn peek_i16(&self) -> Result<i16> {
        Ok(i16::from_be_bytes(
